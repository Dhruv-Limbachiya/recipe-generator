...
class GenerateRecipeViewModel @Inject constructor(
     ...
     */
    fun generateRecipe() = viewModelScope.launch {
        ...
        generatedRecipeFlow.collectLatest { resource ->
            ...
            when (resource) {
                is Resource.Error -> updateGenerateRecipeState(
                ...
                is Resource.Loading -> updateGenerateRecipeState(
                    _generateRecipeState.value.copy(
                        isLoading = true
                    )
                )

                is Resource.Success -> updateGenerateRecipeState(
                    _generateRecipeState.value.copy(
                        isLoading = false,
